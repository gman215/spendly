...
- **Create:** `templates/profile.html` — full profile page extending `base.html`; contains four sections:
  1. **User info card** — avatar initials, name, email, member-since date. This section renders the
     *real* signed-in user via the `current_user` context processor added in Step 3 (`app.py`), not
     hardcoded values — the helper already exists and works, and hardcoding it here would only have
     to be undone in Step 5.
...
- **Modify:** `templates/base.html` — turn the nav greeting into a link to `/profile` so the page is
  reachable while signed in (required by the navbar item in the Definition of done).

## Files to change
- `app.py` — replace the `/profile` stub with a real view function that:
  - Redirects unauthenticated users to `/login`
  - Passes hardcoded context variables to `profile.html`
- `templates/base.html` — nav greeting becomes a `/profile` link
- `static/css/style.css` — new "Profile page" section (cards, stat tiles, transaction table,
  category badges, progress bars) plus responsive rules
...
- No inline styles, except data-driven bar widths — a percentage computed per row cannot live in a
  static stylesheet without a class per possible width, and `templates/landing.html` already sets
  `style="width: …%"` on its mock bars
- Authentication guard: check `session.get("user_id")`; if absent, `redirect(url_for("login"))`
- The summary stats, transaction rows, and category breakdown must be hardcoded Python dicts/lists
  in `app.py` — no DB queries for those in this step. The user info card is the exception: it reads
  the signed-in user through the existing `current_user` context processor.
